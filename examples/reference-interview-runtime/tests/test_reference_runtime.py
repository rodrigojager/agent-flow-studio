...
def test_reference_session_flow_idempotency_transcript_and_events(tmp_path):
    ...
    with client.stream("GET", f"/sessions/{session_id}/events/stream?from_seq=1&max_events=1") as stream:
    ...
    with client.websocket_connect(f"/sessions/{session_id}/events/ws?from_seq=1&max_events=1") as websocket:
    ...
    completed_events = client.get(f"/sessions/{session_id}/events").json()
    assert "post_finish_completed" in [item["event_type"] for item in completed_events]


def test_reference_metadata_exposes_sanitized_worker_retention_policy(tmp_path):
    client = _client(
        tmp_path,
        env_overrides={
...
def test_reference_job_cleanup_previews_and_deletes_only_old_terminal_jobs(tmp_path):
    client = _client(tmp_path)
    ...
    assert client.get(f"/sessions/{session_id}/transcript").status_code == 200
    events = client.get(f"/sessions/{session_id}/events").json()
...
def test_reference_ollama_missing_model_returns_prescriptive_fallback(tmp_path, monkeypatch):
    class FakeResponses:
    ...
    create_resp = client.post(
        "/sessions",
        headers={"Idempotency-Key": "ollama-create"},
        json={"metadata": {"source": "pytest"}, "max_turns": 2},
    )
    assert create_resp.status_code == 200
    session_id = create_resp.json()["session"]["session_id"]
    assert client.post(
        f"/sessions/{session_id}/start",
        headers={"Idempotency-Key": "ollama-start"},
        json={},
    ).status_code == 200

    turn_resp = client.post(
        f"/sessions/{session_id}/turn",
        headers={"Idempotency-Key": "ollama-turn"},
        json={"user_message": "Vamos testar o modelo local."},
    )
    assert turn_resp.status_code == 200
    assistant_text = turn_resp.json()["assistant_message"]["text"]
    assert "ollama pull qwen3:8b" in assistant_text
    assert "docker compose --profile model-setup up ollama-pull-qwen3-8b" in assistant_text

    events = client.get(f"/sessions/{session_id}/events").json()
    llm_event = next(item for item in events if item["event_type"] == "llm_called")
    assert llm_event["payload"]["provider"] == "ollama"
    assert llm_event["payload"]["fallback_reason"] == "local_model_missing"
    assert llm_event["payload"]["setup_command"] == "ollama pull qwen3:8b"
    assert llm_event["payload"]["docker_setup_command"] == "docker compose --profile model-setup up ollama-pull-qwen3-8b"


def test_reference_turn_stream_emits_tokens_and_reuses_idempotency(tmp_path):
    client = _client(tmp_path)
...
def test_reference_turn_stream_ws_emits_events_and_reuses_idempotency(tmp_path):
    client = _client(tmp_path)
    ...
    with client.websocket_connect(
        f"/sessions/{session_id}/turn/stream/ws?user_message=Mensagem%20com%20stream%20WS.&idempotency_key=ws-turn-stream"
    ) as websocket:
        ...
        while True:
            ...
            if item["event"] in {"turn_completed", "turn_error"}:
                break
        closed = websocket.receive_json() if stream_events[-1]["event"] != "stream_closed" else None
        if closed is not None:
            stream_events.append(closed)

    assert stream_events[0]["event"] == "turn_started"
    assert any(item["event"] == "token" for item in stream_events)
    assert any(item.get("data", {}).get("source") == "llm_callback" for item in stream_events if item["event"] == "token")
    assert any(item["event"] == "turn_completed" for item in stream_events)
    assert stream_events[-1]["event"] == "stream_closed"
    assert any(
        item.get("data", {}).get("reason") == "turn_completed"
        for item in stream_events
        if item["event"] == "stream_closed"
    )

    duplicate_turn = client.post(
        f"/sessions/{session_id}/turn",
        headers={"Idempotency-Key": "ws-turn-stream"},
        json={"user_message": "Mensagem com stream WS."},
    )
    assert duplicate_turn.status_code == 200
    assert duplicate_turn.json()["assistant_message"]["code"] == "ECHO"


def test_reference_legacy_api_key_still_has_full_access(tmp_path):
    client = _client(
        tmp_path,
        env_overrides={
            "AUTH_ENABLED": "true",
    ...
    assert client.post("/sessions", json={"max_turns": 2}).status_code == 403
    create_resp = client.post(
        "/sessions",
...
def test_reference_scoped_api_keys_enforce_runtime_permissions(tmp_path):
    scoped_keys = (
    ...
    with client.stream(
        "GET",
        f"/sessions/{session_id}/events/stream?from_seq=1&max_events=1",
    ) as stream:
        assert stream.status_code == 403

    with client.stream(
        "GET",
        f"/sessions/{session_id}/events/stream?from_seq=1&max_events=1&api_key=reader-key",
    ) as stream:
        assert stream.status_code == 200
    ...
    with client.websocket_connect(
        f"/sessions/{session_id}/events/ws?from_seq=1&max_events=1&api_key=reader-key"
    ) as websocket:
...
def test_reference_agent_scoped_api_keys_are_limited_to_current_agent(tmp_path):
    scoped_keys = json.dumps(
        {
            "agent-reader-key": [
                "agents:reference-interview:metadata:read",
                "agents:reference-interview:sessions:read",
            ],
            "agent-operator-key": ["agents:reference-interview:sessions:*"],
            "agent-auth-key": ["agents:reference-interview:auth:read"],
    ...
    assert keys_payload["agent_id"] == "reference-interview"
...
def test_reference_api_keys_support_local_expiration_metadata(tmp_path):
...
def test_reference_api_keys_support_local_revocation_metadata(tmp_path):
...
def test_reference_api_keys_file_supports_local_rotation_without_restart(tmp_path):
    keys_path = tmp_path / "api-keys.json"
    keys_path.write_text(
        json.dumps(
            {
    ...
    assert client.post("/sessions", headers={"X-Agent-API-Key": "reader-file-key"}, json={"max_turns": 2}).status_code == 403
    assert client.post("/sessions", headers={"X-Agent-API-Key": "disabled-key"}, json={"max_turns": 2}).status_code == 403
...
def test_reference_auth_rate_limit_and_audit_log(tmp_path):
...
def test_reference_worker_processes_pending_jobs(tmp_path):
    client = _client(tmp_path)
...
def test_reference_worker_can_run_governed_cleanup_policy(tmp_path):
    client = _client(tmp_path)
...
def test_reference_job_claim_lease_prevents_duplicate_multiworker_and_allows_takeover(tmp_path):
    client = _client(tmp_path)
    ...
    session_id = create_resp.json()["session"]["session_id"]
    assert client.post(f"/sessions/{session_id}/start", headers={"Idempotency-Key": "lease-start"}, json={}).status_code == 200
    assert client.post(
        f"/sessions/{session_id}/turn",
        headers={"Idempotency-Key": "lease-turn"},
        json={"user_message": "Resposta para testar lease."},
    ).status_code == 200
    assert client.post(f"/sessions/{session_id}/finish", headers={"Idempotency-Key": "lease-finish"}, json={}).status_code == 200
...
def test_reference_job_batch_endpoints_run_pending_jobs(tmp_path):
    client = _client(tmp_path)
...
def test_reference_worker_retries_and_exposes_manual_retry(tmp_path):
    client = _client(tmp_path)
    ...
    assert retry_metrics["pending_due"] == 1
    retry_events = client.get(f"/sessions/{session_id}/events").json()
...
def test_reference_job_schedule_endpoint_delays_due_work(tmp_path):
    client = _client(tmp_path)
...
def test_reference_recurring_job_schedule_enqueues_due_work(tmp_path):
    client = _client(tmp_path)
...
def test_reference_event_job_schedule_enqueues_only_when_event_is_triggered(tmp_path):
    client = _client(tmp_path)
...
def test_reference_cron_job_schedule_uses_cron_expression(tmp_path):
    client = _client(tmp_path)
...
def test_reference_compose_includes_worker_service():
...
def test_external_safety_provider_blocks_when_local_allows(tmp_path, monkeypatch):
    calls = []
    ...
    create_resp = client.post("/sessions", headers={"Idempotency-Key": "external-safe-create"}, json={"max_turns": 2})
    session_id = create_resp.json()["session"]["session_id"]
    assert client.post(
        f"/sessions/{session_id}/start",
        headers={"Idempotency-Key": "external-safe-start"},
        json={},
    ).status_code == 200
