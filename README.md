...
- Editor visual de JSON Schema na aba `Arquivos`, com propriedades recursivas, required, enum, arrays/objetos aninhados, `$defs`, `$ref`, `additionalProperties`, composições `oneOf`/`allOf`/`anyOf`, validação semântica local de refs, required, arrays, enums e composições, navegação por diagnóstico e correções guiadas iniciais para criar `$defs`, limpar `required`, preencher `items` e deduplicar listas, sincronizados com o JSON bruto.
...
- Evoluir edição visual de prompts, schemas e metadados para correção assistida avançada e padrões reutilizáveis.
...
- Evoluir o catálogo local para biblioteca compartilhável opcional, governança de pacotes reutilizáveis e edição avançada de schemas internos.
