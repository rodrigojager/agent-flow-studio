...
- Editor visual de JSON Schema na aba `Arquivos`, com propriedades recursivas, required, enum, arrays/objetos aninhados, `$defs`, `$ref`, `additionalProperties` e composições `oneOf`/`allOf`/`anyOf` sincronizados com o JSON bruto.
...
- Evoluir edição visual de prompts, schemas e metadados para validação semântica profunda e padrões reutilizáveis.
