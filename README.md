...
- Editor visual de JSON Schema na aba `Arquivos`, com propriedades recursivas, required, enum, arrays/objetos aninhados, `$defs`, `$ref`, `additionalProperties`, composições `oneOf`/`allOf`/`anyOf`, validação semântica local de refs, required, arrays, enums e composições e correções guiadas iniciais para criar `$defs`, limpar `required`, preencher `items` e deduplicar listas, sincronizados com o JSON bruto.
...
- Evoluir edição visual de prompts, schemas e metadados para navegação por diagnóstico, correção assistida avançada e padrões reutilizáveis.
