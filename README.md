...
- Editor visual de JSON Schema na aba `Arquivos`, com propriedades recursivas, required, enum, arrays/objetos aninhados, `$defs`, `$ref`, `additionalProperties`, composições `oneOf`/`allOf`/`anyOf` e validação semântica local de refs, required, arrays, enums e composições, sincronizados com o JSON bruto.
...
- Evoluir edição visual de prompts, schemas e metadados para validação semântica avançada, navegação/correção assistida e padrões reutilizáveis.
