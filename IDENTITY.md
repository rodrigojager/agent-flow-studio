...
Baseline manual, flow spec inicial com catálogo de adapters LLM, manifesto de agrupamento inicial, Builder API mínima, Builder UI inicial, edição persistente de flow/prompts/schemas/nós/arestas/LLM, validação visual rica, import/export de workspace de flow, preview/download de artefatos gerados, operação visual inicial de `runtime.manifest.json`, sandbox local com logs, porta configurável e listagem de runtimes, codegen Python dirigido por nós/arestas do flow, bundle por manifesto, composição multiagente inicial em processo único e verificação automatizada de paridade implementados. Sandbox avançado, codegen para recursos avançados e recursos avançados ainda não foram implementados.
