...
Baseline manual, flow spec inicial com catálogo de adapters LLM, manifesto de agrupamento inicial, Builder API/UI com criação de flows por template, edição persistente de flow/prompts/schemas/nós/arestas/LLM, criação/remoção de prompts e schemas, validação visual rica, import/export de workspace de flow, preview/download de artefatos gerados, operação visual inicial de `runtime.manifest.json`, sandbox local com logs, porta configurável e listagem de runtimes, codegen Python dirigido por nós/arestas do flow com `switch`, `human_input`, `http_request`, `transform_json`, `database_query`, `database_save`, `file_extract` e `rag_retrieval`, bundle por manifesto, composição multiagente inicial em processo único e verificação automatizada de paridade implementados. Sandbox avançado e recursos avançados como approval, scoring e analytics ainda não foram implementados.
