...
- editor visual de schemas com propriedades comuns, `$defs`, `$ref`, `additionalProperties`, composições `oneOf`/`allOf`/`anyOf`, validação semântica local de refs, required, arrays, enums e composições, navegação por diagnóstico e correções guiadas iniciais;
...
- pendente evoluir para biblioteca compartilhável real, governança de pacotes, correção assistida avançada e editor avançado de schemas internos.
