...
Criar uma fábrica de agentes de IA orientados a fluxo, estado e API. O usuário desenha um fluxo visual, configura prompts, schemas, regras e integrações, valida o fluxo, gera um runtime LangGraph + FastAPI, testa em sandbox, empacota em Docker e consome o agente por HTTP.
...
- runtime gerado: Python, FastAPI, LangGraph, Pydantic, SQLAlchemy, Redis, OpenAI SDK;
...
### Fase 2: Codegen LangGraph + FastAPI

Gerar o runtime Python a partir do flow.
...
- o runtime gerado se comporta como o baseline sem copiar diretamente os arquivos do baseline.
...
Permitir gerar runtime pela interface.
...
- o builder visual gera o runtime equivalente ao baseline.

### Fase 5: Sandbox

Testar agentes gerados rapidamente.
...
- o usuário consegue testar sem montar comandos manualmente.
