...
- `npm run test:codegen` cobre execução real de nó `code` TypeScript por arquivo, com `codeDependencies` materializado em `app/code/package.json`, além dos testes já existentes de Python e JavaScript.
...
- Adapters externos para contratos de código customizado fora dos executores nativos Python/JavaScript/TypeScript, incluindo HTTP/MCP configurável, sidecar/runtime adapter para outras linguagens, sandbox isolado por nó e UI de logs/erros dedicada no Studio Local.
