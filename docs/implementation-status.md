...
- `npm run test:codegen` cobre execução real de nó `code` TypeScript por arquivo, com `codeDependencies` materializado em `app/code/package.json`, execução real de `codeExecution: "http"` contra servidor HTTP local no pytest gerado, além dos testes já existentes de Python e JavaScript.
...
- Adapters externos para contratos de código customizado fora dos executores nativos Python/JavaScript/TypeScript e HTTP, incluindo MCP configurável, sidecar/runtime adapter para outras linguagens, sandbox isolado por nó e UI de logs/erros dedicada no Studio Local.
...
3. **Adapters de código não nativos (Média/Longo prazo)**
   - adicionar contrato de execução MCP/sidecar;
