...
- Builder API expõe catálogo local reutilizável via `/catalog`, `/catalog/items` e `/flows/{flowId}/catalog/apply`, persistindo itens do workspace em `.agent-flow/catalog/registry.json`, carregando seeds locais de prompt/schema/tools e aplicando prompts/schemas como assets reais ou tools como patch/criação de nó no flow.
- Builder UI possui aba `Catálogo`, com filtro por tipo, refresh, cards de itens locais/built-in, ações para salvar o prompt/schema atual no catálogo e ações para adicionar/aplicar prompt, schema ou tool no flow ou no nó selecionado.
...
- O catálogo local ainda precisa evoluir de prompt/schema/tools iniciais para templates completos de agentes, skills, tools compostas, versionamento/curadoria visual e compartilhamento opcional.
