...
- Builder UI possui aba `Catálogo`, com filtro por tipo, refresh, cards de itens locais/built-in, ações para salvar o prompt/schema atual, salvar o nó selecionado como tool ou skill local, criar flow a partir de template de agente e adicionar/aplicar prompt, schema, tool ou skill no flow ou no nó selecionado.
