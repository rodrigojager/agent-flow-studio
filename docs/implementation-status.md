...
- Ergonomia refinada do canvas ainda precisa de comandos contextuais mais completos; o editor visual de JSON Schema já cobre a primeira camada de `$defs`, `oneOf`, `allOf`, `anyOf`, `$ref`, `additionalProperties`, validação semântica local, navegação por diagnóstico, correção guiada básica e padrões reutilizáveis iniciais, mas ainda precisa evoluir para correção assistida avançada e curadoria governada de schemas complexos.
